...
# Nested State allows JIT compilation of the entire frontend!
...
class DualStreamVisionCortex:
    def __init__(self, key, n_pixels=N_PIXELS, n_csnn_out=256, n_stdp_out=256):
        ...
        if os.path.exists(weights_path):
            ...
            print("Dual-Stream Vision: Loaded Frozen CSNN Anchor successfully.")
        else:
            dummy_ev = jnp.zeros((1, n_pixels * 2))
            dummy_tof = jnp.zeros((1, 1))           
            self.frozen_params = self.csnn.init(k_csnn, dummy_ev, dummy_tof)
            print(f"WARNING: {weights_path} not found. Using random initialized weights.")
    ...
    def __call__(self, state: FusionState, ev_frame, tof_dist, learn=True):
        ...
        # Run both routes in parallel using the Time Surface
        # Normalize live ToF to match the training data [0, 1]!
        ...
        # Integrate stdp_features (spike-trace EMA) not raw binary spikes.
