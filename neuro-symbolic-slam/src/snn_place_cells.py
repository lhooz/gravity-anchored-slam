...
# Grid geometry is owned by snn_pose_cann (no cycle: snn_pose_cann does not import this module).
# Importing it here keeps the grid->place projection in sync with the actual CANN, instead of
# duplicating the module sizes as literals (which silently breaks any change to CANN_SIZES).
try:
...
# Place-cell / appearance-hash dimension (kept in sync with FLYHASH_CONFIG['num_bits'] in
# snn_slam_system.py). This is the SINGLE SOURCE OF TRUTH for the place-layer size.
N_PLACE       = 256
RING_N        = 64
GAIN          = 4.0   
CORRECTION_GAIN      = 2.0    
...
# RING_SELF_MATCH_THRESH (It was punishing perfect alignments!)
...
class PlaceCellState(NamedTuple):
    ...
    confidence_counter: jnp.ndarray  # Consecutive-frame counter replaces leaky EMA
...
class PlaceCellNetwork:
    ...
    # Updated k_spikes default to 16 to match the 512-dim expansion
    def __init__(self, key, n_csnn=N_CSNN, n_stdp=N_STDP, n_depth=N_DEPTH, fov_deg=90.0, n_pose=N_POSE, n_place=N_PLACE, k_spikes=16):
        ...
        # Derived from CANN_SIZES so a change to the grid resolution propagates here.
        # (Previously hardcoded as [11,13,17] / ends=[...,579], which made W_dg permanently
        #  579-row and crashed get_place_barcode for any other grid size.)
    ...
    def _updateHebbian(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, ring_bump, heading, angular_vel=None, confidence=None):
        ...
        # Global decay applied to ALL sequence weights (not gated by active barcode).
    ...
    def compute_confidence_with_gates(self, state: PlaceCellState, vis_csnn, vis_stdp, tof_features, pose_bump, heading, ring_bump):
        ...
        # Removed `is_not_self_matching` from the gate array so perfect overlaps are celebrated!
        is_confident_raw = (
            is_distinctive & is_match & is_anti_aliased & 
            is_heading_plausible & is_mature & is_tof_match 
        )

        # Consecutive-frame counter replaces leaky EMA (α=0.5).
