...
def get_ray_indices(cx, cy, cth, tof_dists, tof_angles, res=0.04, grid_size=50, offset_m=0.0, max_rays=500): 
    ...
    # Pad with -1. This guarantees static array shapes for JIT.
...
def run_live_slam(key):
    ...
    # Graph Data Structures & Keyframing
    graph_poses = []
    graph_odom_edges = []
    node_tof_hits = []       
    loop_closures = []
    loop_offsets_list = []  # Track Relative Transforms
    loop_weights_list = []  # Track Confidence Springs
    
    # The JAX-Native Memory Bank (Replaces the Python Dictionary)
    # Holds a 512-dim barcode for up to 15000 nodes.
    MAX_MAP_NODES = 15000  
    memory_bank = np.zeros((MAX_MAP_NODES, FLYHASH_CONFIG["num_bits"]), dtype=np.float32)
    stdp_memory_bank = {}  # Sparse dict {node_id → W array} replaces ~7.5 GB dense allocation
    ...
    # Unified Right-Column SubGridSpec to prevent overlap and irregular grids
    ...
    # Initialize the memory node scatter plot
    nodes_scatter, = ax_map.plot([], [], marker='.', color='silver', ms=5, alpha=0.2, label="Memory Nodes", zorder=8)
    # Initialize the candidate nodes marker (Large hollow orange circles)
    candidates_scatter, = ax_map.plot([], [], 'o', markeredgecolor='orange', markerfacecolor='none', ms=12, markeredgewidth=2, label="FlyHash candidates", zorder=9)
    ax_map.legend(loc='upper right', fontsize=8)

    learning_indicator = ax_map.text(0.02, 0.98, 'Plasticity: ON', 
    ...
    brain_img = ax_place.imshow(np.zeros((1, FLYHASH_CONFIG["num_bits"])), cmap='magma', aspect='auto', vmin=0, vmax=1.0)
    ...
    # Save GIF dynamically to the workspace root directory
    ...
    # Pass loop=0 to force imageio to write the loop extension block so the GIF loops infinitely on GitHub!
    ...
    # Variables for FlyHash tracking and CLS Autopilot
    active_candidates = []
    surprise = 0.0
    ...
    try:
        while True:
            ...
            if step > 0:
                ...
                # 2nd-Order Midpoint Integration (RK2)
            ...
            # Only freeze STDP at HIGH surprise (≥0.60), not moderate novelty (≥0.30).
            ...
            if is_keyframe:
                ...
                # Save the VISUAL Barcode to the database!
                vis_barcode_np = np.array(debug_gates['Visual_Barcode'][0])
                if current_node_id < MAX_MAP_NODES:  # Guard against OOB after 15k nodes
                    memory_bank[current_node_id] = vis_barcode_np

                # Save the Synaptic Snapshot (now in sparse dict — no memory limit)
                stdp_weights_np = np.array(system_cl.vision_state.stdp_state.W[0])
                stdp_memory_bank[current_node_id] = stdp_weights_np

                lc_success = False
                abort_reason = None  # Tracks gate that aborted LC for differential refractory timer

                # THE LOOP CLOSURE ENGINE (Gated by Surprise!)
                if surprise >= 0.30 and lc_refractory_timer == 0:
                    matched_node = None
                    best_fitness_score = -999.0  # Initialize the tracker here!
                    curr_tof = np.array(tof_t) 
                    is_cross_path = False
                    
                    # Widened exclusion zone from 15→50 nodes (~15m)
                    # 15 nodes at KEYFRAME_DIST=0.30m was only 4.5m — too close!
                    valid_limit = current_node_id - 50
                    
                    if valid_limit > 0:
                        # Query the database using what the robot SEES!
                        vis_barcode_np = np.array(debug_gates['Visual_Barcode'][0])
                        overlaps = np.dot(memory_bank[:valid_limit], vis_barcode_np)
                        valid_candidates = np.where(overlaps >= FLYHASH_CONFIG["match_threshold"])[0]
                        
                        # Save these IDs so the UI thread can plot them!
                        ...
                        if len(valid_candidates) > 0:
                            ...
                            for candidate_nid in valid_candidates:
                                ...
                                # Topological distance (graph hops) replaces
                            ...
                            # print(f"\n 🛡️ BOUNCER REJECTED: Vision confident, but highest Visual Barcode overlap was {best_overlap_score:.1f}/{FLYHASH_CONFIG['active_spikes_k']} spikes!")
                            pass

                    if matched_node is not None:
                        # =======================================================
                        # Sequence-coherence verification (multi-frame)
                        # A single-frame match can be aliased. But 5 CONSECUTIVE
                        # frames matching in ORDER is extremely hard to fake!
                        # =======================================================
                        SEQ_LEN = 5
                        SEQ_OVERLAP_THRESH = FLYHASH_CONFIG["match_threshold"] - 1  # slightly looser for neighbors
                    ...
                    if matched_node is not None:
                        ...
                        if maturity >= LC_MATURITY: 
                            ...
                            if not abort_lc:
                                ...
                                # Grab the current visual sparsity
                                ...
                                if vis_act < 0.08:
                                    ...
                                    # Modulo wrap using the padded length, not N_VIS
                                    y1 = r_real[(peak_idx - 1) % N_PAD]
                                    y2 = r_real[peak_idx] 
                                    y3 = r_real[(peak_idx + 1) % N_PAD]
                                    
                                    # Clip to ±1 px — prevents ±∞ blowup when peak is flat (denom ≈ 0)
                                    denom = 2.0 * (y1 - 2.0 * y2 + y3)
                                    sub_pixel_offset = np.clip((y1 - y3) / (denom - 1e-8), -1.0, 1.0)
                                    
                                    # Negative integer shifts must subtract the padded length
                                ...
                                if not abort_lc and not is_cross_path:
                                    ...
                                    if abs(lc_offset_th) > 0.30:
                                        print(f"\n Aborted: Visual shift too extreme! lc_offset_th = {lc_offset_th:.2f} rad. Preventing violent graph snap.")
                                        matched_node = None
                                        abort_lc = True
                                        abort_reason = 'visual'
                                    ...
                                    # =================================================================
                                    if not abort_lc:
                                        # Use ring-attractor decoded heading (less drift-sensitive)
                                        ...
                                        if vision_contradiction > 0.20:  
                                            print(f"\n Aborted: Visual Hallucination! Vision claims offset {lc_offset_th:.2f} rad, but IMU claims {dth_physical:.2f} rad.")
                                            matched_node = None
                                            abort_lc = True
                                            abort_reason = 'visual'

                                if not abort_lc and not is_cross_path:
                                    # Correct Physical Laser Mapping
                                    ...
                                    if abs(lc_offset_th) > 0.08:
                                        if lc_offset_th > 0 and rotational_signature < -0.10:
                                            print(f"\n Aborted: Directional Contradiction! Vision claims LEFT (+{lc_offset_th:.2f}), but lasers swept RIGHT ({rotational_signature:.2f}).")
                                            matched_node = None
                                            abort_lc = True
                                            abort_reason = 'physical'
                                        elif lc_offset_th < 0 and rotational_signature > 0.10:
                                            print(f"\n Aborted: Directional Contradiction! Vision claims RIGHT ({lc_offset_th:.2f}), but lasers swept LEFT (+{rotational_signature:.2f}).")
                                            matched_node = None
                                            abort_lc = True
                                            abort_reason = 'physical'
                            ...
                            if not abort_lc:
                                if is_cross_path:
                                    ...
                                    if np.sum(valid_mask) >= 3:
                                        ...
                                        if det_AtA < 0.10: 
                                            print(f"\n Aborted: Matrix Singularity! Det(AtA) = {det_AtA:.3f}. Geometry is too flat/ambiguous to solve.")
                                            matched_node = None
                                            abort_lc = True
                                        ...
                                        if not abort_lc:
                                            ...
                                            # Per-ray geometric tolerance 0.25 m
                                            if np.any(in_range & (residuals > 0.25)):
                                                print(f"\n Aborted: Perceptual Aliasing! Geometry shift couldn't explain all rays (Max residual: {np.max(residuals[in_range]):.2f}m).")
                                                matched_node = None
                                                abort_lc = True
                                                abort_reason = 'geometric'
                                    else:
                                        print(f"\n Aborted Loop Closure ({current_node_id} -> {matched_node}): Lasers out of range or residuals too high.")
                                        matched_node = None
                                        abort_lc = True
                                        abort_reason = 'geometric'
                            ...
                            if not abort_lc:
                                ...
                                # Scale springs by sqrt(topological distance)
                                ...
                                # ==================================================
                                # Full FlyHash sparse code Memory Update
                                # Update all 16 active columns of the FlyHash memory simultaneously!
                                # ==================================================
                                ...
                                # Calculate the true hardware drift!
                ...
                if is_confident[0] and not lc_success:
                    # Differential refractory — geometric failures recover faster than hallucinations
                    ...
                    if dist < KEYFRAME_DIST and ang < KEYFRAME_ANG and current_node_id > 0:
                        ...
                        node_tof_hits.pop()
                        if current_node_id < MAX_MAP_NODES:  # Bounds-checked clear
            ...
            if is_keyframe:
                ...
                # Safe edge slice — guards against pruning-induced count divergence
                ...
                # Conditional relaxation — full 3000 iters on LC events, 500 iters every 10th keyframe
                ...
                if lc_success:
                    ...
                    # Soft-blend reconsolidation — retrieved memory blended with current state
                ...
                # ==================================================
                # only plot and print on true snap success
                # ==================================================
                if lc_success and "Debug_Input_CSNN" in debug_gates:
                    ...
                    # Plot the Top 16 Spikes of the FlyHash sparse code!
                    top_k_indices = np.argsort(i_place)[-FLYHASH_CONFIG["active_spikes_k"]:]
                    for idx in top_k_indices:
                        axs[3, 0].axvline(idx, color='gold', linestyle='--', lw=1, alpha=0.5)
                        
                    axs[3, 0].set_title(f"FlyHash sparse code Activation (Top {FLYHASH_CONFIG['active_spikes_k']} Spikes)", fontweight='bold')
            ...
            # Vectorized smooth trajectory reconstruction (replaces O(n) Python loop)
            ...
            if step % 15 == 0:
                ...
                # Extract, align, and plot the memory nodes
                if len(graph_poses) > 0:
                ...
                # Extract and plot the active FlyHash candidate nodes
                if len(active_candidates) > 0:
                    ...
                    # Clear the list so the rings vanish on the NEXT UI frame!
                    active_candidates = []
                else:
                    candidates_scatter.set_data([], [])
                ...
                # Gauge-invariant ATE — apply Umeyama alignment per path before computing error
                ...
                pc_act = np.array(r_place[0]).reshape(1, FLYHASH_CONFIG["num_bits"])
                ...
                # Append to a sliding window of the last 300 frames (DPI=55 for publication aesthetic)
                buf = io.BytesIO()
        ...
        # Write the sliding window FIFO queue frames to the gif!
