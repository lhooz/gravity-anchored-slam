...
# Lengthened so associations survive across frames!
...
class STDPLayer:
    ...
    def apply_batch(self, W, e_pre, e_post, spike_pre_all, spike_post_all):
        ...
        # Activity-Dependent Synaptic Scaling (L1 Normalization)
...
class VisionSTDP:
    ...
    def __call__(self, state: VisionSTDPState, time_surface, tof_dist, learn=True):
        ...
        # Normalize by per-sample max so feature scale stays bounded [0, 1]
...
def evaluate_vision_stdp(key, events, tof_dists, learn=True):
    ...
    # Initialize mock_time_surface before the loop!
    ...
    if W_change < 1e-4:
        print("    WARNING: STDP IS DEAD (No weight change detected).")
    else:
